...
TOPDIR=${UBCESLAB_SWENV_PREFIX:?undefined}/libs/cppunit-dbg
...
(CFLAGS="-g" CXXFLAGS="-D_GLIBCXX_DEBUG -D_GLIBCXX_DEBUG_PEDANTIC" ./configure --prefix=$CPPUNIT_DIR 2>&1 && touch build_cmd_success) | tee configure.log
...
cd $CURRENT_DIR
MODULEDIR=$UBCESLAB_SWENV_PREFIX/apps/lmod/derived_modulefiles/${COMPILER:?undefined}/${COMPILER_VERSION:?undefined}/modulefiles/cppunit-dbg
mkdir -p $MODULEDIR
...
echo "local name = \"cppunit-dbg\"" >> $MODULEDIR/$CPPUNIT_VERSION.lua
